...
Shader * load_normal_shader (const char * fs, const char * func, const char * file, int line)
{
  ...
  CUDA_CHECK (hipModuleGetFunction (&shader->kernel, shader->module, func));
  return shader;
}
...
void post_setup_shader (Shader * shader, External * externals)
{
  ...
  assert (ssbo);
  CUDA_CHECK (hipMemcpyHtoDAsync (shader->_data, &ssbo, sizeof (ssbo), stream));
  ...
  for (const MyUniform * g = shader->uniforms; g && g->type; g++) {
    ...
    case sym_INT: case sym_FLOAT: case sym_VEC4: case sym_BOOL:
      CUDA_CHECK (hipMemcpyHtoDAsync (g->location, pointer, g->size, stream));
      break;
    case sym_LONG: {
      int p[g->nd];
      long * data = pointer;
	...
	p[i] = data[i];
      CUDA_CHECK (hipMemcpyHtoDAsync (g->location, p, g->size, stream));
      break;
    }
#if SINGLE_PRECISION
...
int run_shader (const Shader * shader, const RegionParameters * region)
{
  ...
  if (region->n.x == 1 && region->n.y == 1) {
    ...
    CUDA_CHECK (hipMemcpyHtoDAsync (shader->csOrigin, csOrigin, 2*sizeof(int), stream));
